...
router.get("/:id", (req, res) => {
  ...
  if (found) {
    ...
    res
      .status(400)
      .json({ message: `pizza Not found with Id ${req.params.id}` });
  }
});
  ...
  if (found) {
    ...
    pizzas.forEach((pizza) => {
      if (pizza.id === req.params.id) {
        ...
        pizza.toppings = updatepizza.toppings
          ? updatepizza.toppings
          : pizza.toppings;
        pizza.ingrediants = updatepizza.ingrediants
          ? updatepizza.ingrediants
          : pizza.ingrediants;
...
router.delete("/:id", (req, res) => {
  ...
  if (found) {
    ...
    res
      .status(400)
      .json({ message: `pizza details found with Id ${req.params.id}` });
  }
});
